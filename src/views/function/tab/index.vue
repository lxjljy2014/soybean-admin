...
<template>
  <NSpace vertical :size="16">
    <NCard title="标签页操作" :bordered="false" size="small" segmented class="card-wrapper">
      ...
      <NSpace>
        <NButton @click="tabStore.removeActiveTab">关闭当前标签页</NButton>
        <NButton @click="tabStore.removeTabByRouteName('about')">关闭"关于"标签页</NButton>
      </NSpace>
      <NDivider title-placement="left">添加多标签页</NDivider>
      <NSpace>
        <NButton @click="routerPushByKey('function_multi-tab')">跳转到多标签页页面</NButton>
        <NButton @click="routerPushByKey('function_multi-tab', { query: { a: '1' } })">
          跳转到多标签页页面(带有查询参数)
        </NButton>
      </NSpace>
    </NCard>
    <NCard title="标签页标题" :bordered="false" size="small" segmented class="card-wrapper">
      ...
      <NButton type="error" ghost class="w-80px" @click="resetTabLabel">重置</NButton>
    </NCard>
  </NSpace>
</template>
